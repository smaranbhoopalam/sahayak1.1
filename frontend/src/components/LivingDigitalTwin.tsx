          ...
          <div className="lg:col-span-7 glass-panel p-6 sm:p-8 rounded-3xl border border-slate-200 bg-white space-y-6 shadow-md">
            ...
            <div className="relative bg-slate-50/70 p-6 pt-10 rounded-2xl border border-slate-200 flex flex-col items-center justify-center min-h-[320px]">
              ...
              <div className="relative z-10 flex flex-col sm:flex-row items-center gap-8 w-full justify-around">
                ...
                {/* SVG Silhouette */}
                <div className="relative w-36 h-64 mt-6">
                  <svg viewBox="0 0 100 200" className="w-full h-full filter drop-shadow-md">
                    <circle 
                      cx="50" cy="22" r="14" 
                      fill={bodyStatusColor === 'emerald' ? '#d1fae5' : bodyStatusColor === 'amber' ? '#fef3c7' : '#ffe4e6'} 
                      stroke={bodyStatusColor === 'emerald' ? '#059669' : bodyStatusColor === 'amber' ? '#d97706' : '#e11d48'} 
                      strokeWidth="2" 
                    />
                    <path 
                      d="M 30,44 L 70,44 L 64,115 L 36,115 Z" 
                      fill={bodyStatusColor === 'emerald' ? '#d1fae5' : bodyStatusColor === 'amber' ? '#fef3c7' : '#ffe4e6'} 
                      stroke={bodyStatusColor === 'emerald' ? '#059669' : bodyStatusColor === 'amber' ? '#d97706' : '#e11d48'} 
                      strokeWidth="2" 
                    />
                    {inputs.painScore > 4 && (
                      <circle cx="50" cy="70" r={inputs.painScore * 2} fill="rgba(225,29,72,0.2)" stroke="#e11d48" strokeWidth="1.5" className="animate-ping" />
                    )}
                    <path d="M 28,45 L 14,100 M 72,45 L 86,100" stroke={bodyStatusColor === 'emerald' ? '#059669' : '#e11d48'} strokeWidth="6" strokeLinecap="round" />
                    <path d="M 40,115 L 38,185 M 60,115 L 62,185" stroke={bodyStatusColor === 'emerald' ? '#059669' : '#e11d48'} strokeWidth="8" strokeLinecap="round" />
                  </svg>
