...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Drawer */}
      <aside
        className={`fixed lg:sticky top-0 left-0 z-50 lg:z-30 w-64 h-screen lg:h-[calc(100vh-68px)] bg-white border-r border-slate-200/80 p-4 overflow-y-auto transition-transform duration-300 ease-in-out flex flex-col justify-between ${
          isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        <div>
          {/* Doctor / User Profile Card */}
          <div className="mb-6 p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 shadow-2xs">
            <div className="flex items-center gap-3">
              <div className={`p-2.5 rounded-xl ${currentRoleInfo.statusColor} text-white shrink-0 relative shadow-sm`}>
                <InfoIcon className="w-5 h-5" />
                <span className={`absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full ${currentRoleInfo.statusDot} ring-2 ring-white animate-pulse`} />
              </div>
              <div className="min-w-0 flex-1 text-left">
                <div className="text-sm font-bold tracking-tight text-slate-900 truncate">{currentRoleInfo.name}</div>
                <div className="text-[10px] text-slate-500 font-semibold truncate mt-0.5">{currentRoleInfo.credential}</div>
                <div className="text-[9px] text-brand-600 font-black uppercase tracking-wider mt-1 truncate">
